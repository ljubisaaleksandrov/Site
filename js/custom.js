...
function adjustHeader(){
	...
	var menuElement = $("#brickhaus-menu");
	var logoElement = $(".logo");

	var menuWidth = $(menuElement).width();
	var menuLeft = $(menuElement).position().left;
	var logoLeft = $(logoElement).position().left;
		
	if($(window).width() < 992 || menuWidth + menuLeft >= logoLeft){
		var headerElement = $(".header");
		var menuElement = $("#brickhaus-menu");
		var logoElement = $(".logo");
		
		var logoMarginTop = 10;
		var logoMarginBottom = 40;
		
		var menuHeight = $(menuElement).height();
		var logoTop	= parseInt($(logoElement).css('top'));
		var logoHeight = $(logoElement).height();
		var headerHeight = $(headerElement).height();
		var offset = menuHeight - logoTop;

		$(logoElement).css({'top': menuHeight + logoMarginTop + 'px' });
		$(headerElement).css({'height': menuHeight + logoMarginTop + logoHeight + logoMarginBottom + 'px' });
	}
	else{
		$(headerElement).css({'height:': '200px'});
		$(logoElement).css({'top:': '106px'});
	}
}
